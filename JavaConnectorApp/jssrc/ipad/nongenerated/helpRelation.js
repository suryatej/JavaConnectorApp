...
function serviceHelpRelation() {
    ...
    if (channel === "tablet") frmKeywords.lblInfo.setVisibility(false);
...
function helpRelationCallback(status, mysqlHelpRelationData) {
    if (status == 400) {
        if (mysqlHelpRelationData["opstatus"] == 0) {
            ...
            if (channel === "tablet") frmKeywords.segHelptopic.setData(hlArray);
            else {
...
function serachOption() {
    ...
    if (flag == 0) navToFrmCategory();
    else navToFrmKeyword();
}

function navPopupSrch() {
    var curFrm = kony.application.getCurrentForm();
    var popupSettingsContext = {
        "widget": curFrm.btnSearch,
        "anchor": "bottom",
